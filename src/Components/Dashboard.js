...
class Dashboard extends Component {
	...
	handleAlphaSort() {
		...
		if (alpha === 'reverse' || alpha === '') {
			this.setState({ alpha: 'alpha', score: '', snackbar: true, message: 'Sorted in alphabetical order' });
		}
		else {
			this.setState({ alpha: 'reverse', score: '', snackbar: true, message: 'Sorted in reverse alphabetical order' });
		}
	...
	handleScoreSort() {
		...
		if (score === 'reverse' || score === '') {
			this.setState({ score: 'score', alpha: '', snackbar: true, message: 'Sorted by marks low to high' });
		}
		else {
			this.setState({ score: 'reverse', alpha: '', snackbar: true, message: 'Sorted by marks high to low' });
		}
		setTimeout(() => this.setState({snackbar: false}), 2000)
	}

	render() {
		const { value, alpha, score, snackbar, vertical, horizontal, message } = this.state;
